...
export default defineEventHandler(async (event) => {
    ...
    let data = await prisma.article.update({
        where: {
    ...
    if (!data)
        return sendError(event, createError({statusCode: 401, statusMessage: 'An error occurred while unpublishing this article. Try again later.' }));
