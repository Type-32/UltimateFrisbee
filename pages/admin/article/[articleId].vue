...
async function onSubmit (event: FormSubmitEvent<Schema>) {
    try {
        const data = await $util.editArticle(articleId, state.title as any as string, state.description as any as string, state.content as any as string, articlePublished.value)
        if(!data)
            ...
            <div class="h-full flex flex-col px-10 w-full gap-5 py-10">
                ...
                <UForm :schema="schema" :state="state" class="grid grid-cols-2 gap-5" @submit="onSubmit">
                    ...
                    <UFormGroup label="Content" name="content" class="col-span-2">
                        <TiptapEditor class="col-span-2" v-model="content" :disabled="loadingPage"/>
                    </UFormGroup>
                </UForm>
            </div>
